...
function puffTexture(coreAlpha: number, coreStop: number): HTMLCanvasElement {
 ...
 */
export class RocketLaunchAnimation implements SpinnerAnimation {
  ...
  private readonly rockets: MeshHandle[] = [];
  private readonly smoke: MeshHandle[] = [];
  private readonly fire: MeshHandle[] = [];
  ...
  mount(target: HTMLElement): void {
    ...
    const rocketMesh: Mesh = pyramid(1, ROCKET_COLORS);
  ...
  private updateBlends(dt: number, progress: number, now: number, exiting: boolean): void {
  ...
  private ascentDistance(la: number): number {
    const seconds = la / 1000;
    return 0.5 * ASCENT_G * seconds * seconds;
  }

  /** Rocket center, nose direction, and roll `la` ms into its climb. */
  private ascentPose(i: number, homeX: number, la: number): { pos: Vec2; dir: Vec2; roll: number } {
  ...
  private renderAscent(i: number, homeX: number, la: number, halfWidth: number): void {
    const { pos, roll } = this.ascentPose(i, homeX, la);
    if (pos.y > HALF_HEIGHT + 0.4 || Math.abs(pos.x) > halfWidth + 0.4) return; // gone
    const transform = this.rockets[i].transform;
  ...
  private emitFire(i: number, homeX: number, la: number, cursor: number): number {
