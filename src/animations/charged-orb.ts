...
const MINI_TRANSPARENCY = { mode: "two-sided", frontOpacity: 0.68, backOpacity: 0.87 } as const;
...
const PARKED: Vec3 = { x: 0, y: 0, z: 50 };

// A glossy white highlight over the cyan orbs, for a charged, energetic sheen.
const ORB_MATERIAL: Material = { specular: [1, 1, 1], shininess: 28 };
 ...
 * stream per orb.
 */
export class ChargedOrbAnimation implements SpinnerAnimation {
  ...
  mount(target: HTMLElement): void {
    ...
    for (let i = 0; i < MINIS; i++) {
      const mesh: Mesh = icosphere(1, 1, MINI_COLORS[i % MINI_COLORS.length], ORB_MATERIAL);
  ...
  private updateSpread(dt: number): void {
  ...
  private baseAngleAt(t: number): number {
  ...
  private reenterStart(): number {
    return this.allOutAt + EXTRA_PAUSE_MS + EXTRA_SPIN_MS;
  }

  private miniSample(index: number, t: number): MiniSample | undefined {
  ...
  private centerScale(now: number, t: number): number {
    if (this.allOutAt !== Infinity) {
